...
include_match = re.compile(r'^\s*include\s+"(?P<filename>[\w./\\]+)"', flags=re.M).match
    ...
    for line in thrift_content.splitlines():
        ...
        if m is not None:
            ...
            for p in include_dirs:
                ...
                raise FileNotFoundError('could not find {} in {}'.format(filename, os.pathsep.join(include_dirs)))
        else:
            lines.append(line)
    return '\n'.join(lines)
